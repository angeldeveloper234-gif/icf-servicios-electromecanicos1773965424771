            ...
            <div className="container mx-auto px-6">
                <div className="flex flex-col md:flex-row justify-between items-end mb-20">
                    <div className="max-w-2xl">
                        <p className="text-[#E07B2A] font-sans text-xs tracking-[0.3em] uppercase mb-4 font-black">
                            Nuestros Servicios
                        </p>
                        <h2 className="text-5xl md:text-7xl font-display text-white leading-tight tracking-widest">
                            TRABAJO QUE <span className="text-[#E07B2A]">DURA</span>
                        </h2>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/5 border border-white/5">
                    {services.map((service, index) => (
                        ...
                        >
                            {/* Service Number */}
                            <span className="font-mono text-[10px] text-[#E07B2A] mb-6 block opacity-60 tracking-widest">
                                SERVICIO {service.id}
                            </span>
                            ...
                            <h3 className="text-3xl font-display text-white mb-6 group-hover:text-[#E07B2A] transition-colors tracking-wider">
                                {service.title}
                            </h3>

                            <div className="space-y-4 mb-8">
                                <p className="text-[#E07B2A] font-sans font-bold text-sm tracking-wide italic">
                                    {service.problem}
                                </p>
                                ...
                                <div className="pt-3">
                                    <p className="text-white font-sans font-medium text-sm flex items-center gap-2">
                                        <span className="w-3 h-[1px] bg-[#E07B2A]"></span>
                                        {service.solve}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="pt-6 border-t border-white/5">
                                ...
                                <p className="text-white font-sans text-base group-hover:text-[#E07B2A] transition-colors font-medium">
                                    {priceMap[service.price] || service.price}
                                </p>
                            </div>

                            {/* Hover accent bar */}
                            <div className="absolute bottom-0 left-0 w-0 h-[2px] bg-[#E07B2A] group-hover:w-full transition-all duration-700" />
                        </motion.div>
                    ))}
                </div>
            </div>
